...
**How to apply:** Set the Vercel build variable `VITE_API_BASE_URL` to the deployed API URL ending in `/api`; configure the API's `ALLOWED_ORIGINS`, production `SESSION_SECRET`, admin allowlist, and cross-site cookies when the API and Vercel frontend use different domains.